...
const MiddleScreen = () => {
    ...
    return (
        <div className='w-8/12 px-5 py-8 bg-[#F7FAFF]'>
            ...
            {/* For Cards data */}
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-3 gap-y-5 lg:gap-y-0 justify-between w-full mt-5'>
                {
            ...
            <div className='mt-8'>
                ...
                {/* for icons and data */}
                <div className='flex flex-col sm:gap-y-5 lg:flex-row lg:w-full justify-around border border-white bg-white rounded-2xl px-5 py-5 mt-3'>
                    {
                        quickActionItems.map((item, index) => (
                            <div key={item.id} className='flex flex-col lg:flex-row md:gap-x-24 lg:gap-x-16'>
                                <div className='flex flex-col items-center gap-y-2'>
                                    <img src={item.icon} alt='icon' />
                                    <p className='font-roboto font-medium text-xs'>{item.text}</p>
                                </div>
                                {index !== quickActionItems.length - 1 && <div className='border border-[#CBCBCB] xs:h-0 sm:mx-40 xs:mt-5 lg:mx-0 lg:h-11 lg:mt-0'></div>}
                            </div>
                        ))
                    }
                </div>
            ...
            <div className='mt-8'>
                ...
                <div className='grid grid-col-1 md:grid-cols-2 gap-x-24 mt-4 gap-y-6 md:gap-y-0'>
                    {
